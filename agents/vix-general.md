...
name: vix-general
description: |
  General-purpose coding agent. FleetView's catch-all when no more specific agent fits — use for open-ended coding tasks, debugging sessions, explanations, or multi-step work that doesn't map cleanly to explore/plan/implement. Examples:

  <example>
  Context: User has an open-ended question about their codebase
  user: "Why is the login endpoint returning 403 intermittently?"
  assistant: "I'll use vix-general to investigate."
  <commentary>
  Debugging session with no clear phase — general agent handles it.
  </commentary>
  </example>

  <example>
  Context: User wants help with a task that spans multiple concerns
  user: "Help me refactor this module and update the docs"
  assistant: "I'll use vix-general for this multi-concern task."
  <commentary>
  Mixed task not fitting a single specialized agent.
  </commentary>
  </example>
model: opus
color: white
