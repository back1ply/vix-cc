...
function collectCommentRanges(node, commentTypes, acc) {
    ...
    for (let i = 0; i < node.childCount; i++) {
        ...
        if (child)
            collectCommentRanges(child, commentTypes, acc);
    }
}
export async function minify(content, lang) {
...
export async function minifyFile(filePath, content) {
    await ensureInit();
    const lang = detectLanguage(filePath);
    if (!lang)
        return content; // unsupported — return raw
