...
function makeToolHandlers(cwd: string): Map<string, ToolHandler> {
  ...
  handlers.set('edit_file', async (p) => {
    ...
    if (!content.includes(old)) throw new Error('old_string not found');
...
export async function handleToolChain(args: ToolChainArgs): Promise<string> {
  const { workflow } = args;
...
`;
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', async (line) => {
      ...
      const resp: Record<string, unknown> = handler
        ? await handler(params).then(
            output => ({ output, is_error: false }),
            err => ({ output: String(err), is_error: true })
          )
