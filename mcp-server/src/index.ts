...
import { handleBackgroundBash, killAllJobs, BashJob } from './vix-background-bash.js';
  ...
  tools: [
    {
      ...
      description:
        'Read a file from disk and automatically minify it using Tree-sitter (stripping comments, collapsing whitespace) for token-efficient output. ' +
    ...
    },
    {
      ...
      description:
        'Edit a file through the virtual filesystem. The file is minified with Tree-sitter, the match is performed on the minified representation, and a formatter restores valid source. ' +
    ...
    },
    {
      ...
      description:
        'Spawn a shell command detached and return immediately with a job_id plus paths to a log file and an rc file. ' +
    ...
    },
    {
      name: 'vix_tool_chain',
      description:
        'Execute a Python workflow that chains multiple tool calls (read_file, grep, glob_files, bash, edit_file, write_file, delete_file) in a single round-trip. ' +
      ...
      inputSchema: {
        ...
        properties: {
          workflow: {
            type: 'string',
            description:
              'Python function body (indented). May use read_file, write_file, edit_file, ' +
  ...
  try {
    switch (name) {
      case 'vix_read_minified': {
        const result = await handleReadMinified(args as unknown as Parameters<typeof handleReadMinified>[0], readFiles);
        return ok(result);
      }
      case 'vix_edit_minified': {
        const result = await handleEditMinified(args as unknown as Parameters<typeof handleEditMinified>[0], readFiles);
        return ok(result);
      }
      case 'vix_background_bash': {
        const result = await handleBackgroundBash(args as unknown as Parameters<typeof handleBackgroundBash>[0], bashJobs);
        return ok(result);
      }
      case 'vix_tool_chain': {
        const result = await handleToolChain(args as unknown as Parameters<typeof handleToolChain>[0]);
        return ok(result);
      }
      default:
